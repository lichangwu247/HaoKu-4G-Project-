...
#define MQTT_TASK //使用背景任务处理MQTT消息 bin
...
#define MAX_MESSAGE_HANDLERS 10 /* redefinable - how many subscriptions do you want? *///bin
#endif

enum QoS { QOS0, QOS1, QOS2, SUBFAIL=0x80 };

/* all failure return codes must be negative */
enum returnCode { DISCONNECTED = -3, BUFFER_OVERFLOW = -2, FAILURE = -1, rSUCCESS = 0 };
...
typedef void (*MqttCallback)(void* parm);    //bin
...
typedef struct MQTTClient
{
    ...
    void (*defaultMessageHandler) (MessageData*);
		tConnState  connState; //bin
	  MqttCallback connectedCb;
	  MqttCallback disconnectedCb;
		MqttCallback initedCb;
...
/**
 * Init MQTT client Callback object
 * @param client
 * @param connectedCb
 * @param disconnectedCb
 * @param initedCb
 */
DLLExport void MQTTClientCallbackInit(MQTTClient* c, MqttCallback connectedCb,
